...
1. The React Component "Pay" Button, which will be the button that activate the "Magic Link"
...
### Implementing the React Frontend Component
...
To Send a Response after Operation in Onsuccess to hosted page, use `sendEventToChild` function with the argument response format of:
```
response = {
